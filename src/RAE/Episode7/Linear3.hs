{-# LANGUAGE LinearTypes, BlockArguments #-}

module RAE.Episode7.Linear3 where
...
withCents n f = let (result, Bank ending_money) = f (Bank 0) Token (Cents n) 
                in (result, ending_money)

split :: Int -> Token t %1-> Cents t
      -> (forall t1 t2. Token t1 %1-> Cents t1 -> Token t2 %1 -> Cents t2 -> r) 
      %1-> r
    ...
    let go :: Int -> Bank %1-> Token t %1-> Cents t -> (Int, Bank)
        go n bank0 tok0 money = view tok0 money & \(Ur amount, tok1) ->
                            ...
                            then (n, spend tok1 money bank0)
                                 ...
                                 go (n+1) (spend triangle_tok triangle_money bank0) tok2 money2
